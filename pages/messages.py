...
if "pages" in ROOT: ROOT = os.path.dirname(ROOT)
...
.chat-list { width: 100%; border-right: 1px solid #e0e0e0; background-color: #f8f9fa; overflow-y: auto; height: 75vh;}
...
with col_chat_list:
    st.markdown('<div class="chat-list"><div style="padding: 1rem; font-weight: bold; border-bottom: 2px solid #A87B33; color: #A87B33;">Conversations</div>', unsafe_allow_html=True)
    ...
    for c in all_my_convos:
        ...
        is_active = "active" if st.session_state.selected_chat == c_id else ""
        
        # Make the entire conversation item clickable
        ...
        st.markdown('<hr style="margin:0.2rem; border-color:#e0e0e0;">', unsafe_allow_html=True)
